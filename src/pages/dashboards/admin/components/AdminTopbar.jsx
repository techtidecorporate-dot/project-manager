        ...
        <header className={clsx(
            ...
            <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-3 pl-6 border-l border-[#fa2742]/20">
                    <img
                        src={`https://ui-avatars.com/api/?name=${user?.name}&background=fa2742&color=373833`}
                        alt={user?.name}
                        className="w-10 h-10 rounded-xl object-cover"
                    />
                    <div className="text-right flex flex-col justify-center">
                        <p className="text-sm font-extrabold text-white leading-none mb-1">{user?.name}</p>
                        <p className="text-[9px] font-semibold text-white/70 uppercase tracking-[0.2em] leading-none">{user?.role || 'Admin'}</p>
                    </div>
