...
const DeveloperTopbar = ({ onToggleSidebar }) => {
    const { user } = useAuth();
    ...
    return (
        <header className={clsx(
            ...
            isScrolled ? "shadow-md border-b border-[#453abc]" : "border-b border-transparent"
        )}>
            ...
            <div className="flex items-center space-x-3 md:space-x-5 lg:space-x-6 shrink-0">
                ...
                <div className="flex items-center space-x-2 md:space-x-3 pl-3 md:pl-5 border-l border-[#453abc]/20">
                    <div className="hidden sm:block text-right">
                        <p className="text-xs lg:text-sm font-extrabold text-white leading-none mb-0.5 lg:mb-1 truncate max-w-[120px]">{user?.name || 'Developer'}</p>
                        <p className="text-[8px] lg:text-[9px] font-semibold text-white/70 uppercase tracking-[0.2em] leading-none">{user?.role || 'Developer'}</p>
                    </div>
                    <img
                        src={`https://ui-avatars.com/api/?name=${user?.name || 'Developer'}&background=453abc&color=ffffff`}
                        alt={user?.name}
