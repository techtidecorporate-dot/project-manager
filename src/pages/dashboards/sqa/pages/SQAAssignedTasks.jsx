        ...
        return (
            <div className="space-y-6 mb-12">
                <div className="flex items-center space-x-3 mb-4">
                    ...
                    <h3 className="text-lg font-black uppercase tracking-widest text-[#191a23]/60 italic">
                        {title} Priority Review Queue
                    </h3>
                </div>
                <div className="space-y-6">
                    {items.map((phase, idx) => (
                        <div key={idx} className="bg-white overflow-hidden rounded-[32px] shadow-xl border border-[#191a23]/5 transition-all hover:shadow-2xl">
                            <div className="p-8">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="space-y-1">
                                        <div className="flex items-center space-x-2 text-xs font-black uppercase tracking-widest text-[#453abc]">
                                            <Layout size={14} />
                                            <span>{phase.project?.title}</span>
                                        </div>
                                        <h3 className="text-2xl font-black text-[#191a23]">{phase.name}</h3>
                                        <div className="flex items-center space-x-4 mt-2">
                                            ...
                                            <div className="flex items-center space-x-1 text-xs text-[#191a23]/60 font-bold">
                                                <Calendar size={14} />
                                    ...
                                    <div className="flex flex-col space-y-2">
                                        {phase.status === 'Completed (Dev)' && (
                                            <button
                                                ...
                                                className="flex items-center justify-center space-x-2 px-6 py-3 bg-[#191a23] text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-[#453abc] transition-all active:scale-95 shadow-lg"
                                            >
                                                <PlayCircle size={18} />
                                ...
                                {phase.description && (
                                    <p className="text-[#191a23]/70 bg-[#f5f5f5] p-4 rounded-2xl text-sm italic border border-[#191a23]/5 mb-6">
                                        "{phase.description}"
                                    </p>
                                )}
                                    ...
                                    <div className="mb-6 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-xs font-black uppercase tracking-widest text-blue-600">Deliverable Attached</span>
                                            </div>
                                            <a href={phase.deliverableUrl} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1 text-[10px] font-black text-blue-700 bg-blue-100 px-3 py-1.5 rounded-lg hover:bg-blue-200 transition-colors">
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-[#191a23]">SQA Review Center</h1>
                    <p className="text-[#191a23]/60 font-medium">Debug and approve completed developer work.</p>
                </div>
                <div className="bg-[#191a23]/5 px-4 py-2 rounded-xl">
            ...
            {loading ? (
                <div className="text-center py-20 text-[#191a23]/20 italic font-medium">Loading review tasks...</div>
                ...
                <div className="text-center py-24 bg-white rounded-[40px] border-2 border-dashed border-[#191a23]/10">
                    <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <h3 className="text-xl font-bold text-[#191a23]/40 tracking-tight">Queue is Empty</h3>
                        <p className="text-sm text-[#191a23]/30 max-w-xs mx-auto font-medium">Work will appear here when developers complete their assigned phases.</p>
                    </div>
                </div>
            )}
            ...
            {isErrorModalOpen && (
                <div className="fixed inset-0 bg-[#191a23]/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[40px] p-10 w-full max-w-2xl shadow-2xl animate-in zoom-in duration-300">
                        <div className="flex justify-between items-center mb-8">
                            <div>
                                <h3 className="text-2xl font-black text-[#191a23]">Report Errors</h3>
                                <p className="text-sm text-[#191a23]/50 font-medium">Phase: {selectedPhase?.name}</p>
                            </div>
                            <button
                        ...
                        <div className="max-h-[50vh] overflow-y-auto pr-2 space-y-6 custom-scrollbar">
                            {errorList.map((error, index) => (
                                <div key={index} className="p-6 bg-[#f9fafb] rounded-[32px] border border-[#191a23]/5 relative group">
                                    <div className="grid gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-[#191a23]/40 ml-1">Error Name</label>
                                            <input
                                                type="text"
                                                value={error.name}
                                                onChange={(e) => handleErrorChange(index, 'name', e.target.value)}
                                                className="w-full bg-white px-4 py-3 rounded-2xl outline-none focus:ring-2 focus:ring-red-500 font-bold text-[#191a23] text-sm border border-transparent shadow-sm"
                                                placeholder="e.g. Broken Login Button"
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-[#191a23]/40 ml-1">Description</label>
                                            <textarea
                                                value={error.message}
                                                onChange={(e) => handleErrorChange(index, 'message', e.target.value)}
                                                className="w-full bg-white p-4 rounded-2xl outline-none focus:ring-2 focus:ring-red-500 font-medium text-[#191a23] text-sm resize-none border border-transparent shadow-sm"
                                                rows="2"
                                                placeholder="Describe what's wrong..."
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-[#191a23]/40 ml-1">Evidence URL / Image Path</label>
                                            <div className="relative">
                                                <input
                                                    type="text"
                                                    value={error.evidence}
                                                    onChange={(e) => handleErrorChange(index, 'evidence', e.target.value)}
                                                    className="w-full bg-white pl-10 pr-4 py-3 rounded-2xl outline-none focus:ring-2 focus:ring-red-500 font-medium text-[#191a23] text-sm border border-transparent shadow-sm"
                                                    placeholder="Link to image/video evidence..."
                                                />
                                                <Upload className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#191a23]/30" size={16} />
                                            </div>
                                        </div>
                                    </div>
                                    {errorList.length > 1 && (
                        ...
                        <button
                            ...
                            className="mt-6 w-full py-4 border-2 border-dashed border-[#191a23]/10 rounded-2xl flex items-center justify-center space-x-2 text-[#191a23]/40 font-bold hover:border-[#453abc] hover:text-[#453abc] transition-all group"
                        >
                        ...
                        <div className="mt-10 grid grid-cols-2 gap-4">
                            <button
                                onClick={() => setIsErrorModalOpen(false)}
                                className="py-4 bg-[#f5f5f5] text-[#191a23] font-black rounded-2xl uppercase tracking-widest text-xs hover:bg-[#e5e5e5] transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="py-4 bg-[#453abc] text-[#191a23] font-black rounded-2xl uppercase tracking-widest text-xs hover:opacity-90 shadow-xl transition-all active:scale-95"
                            >
                                Confirm & Reject
                            </button>
                        </div>
