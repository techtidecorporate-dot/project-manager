...
const SQATopbar = ({ onToggleSidebar }) => {
    const { user } = useAuth();
            ...
            <div className="flex items-center space-x-3 md:space-x-5 lg:space-x-6 shrink-0">
                ...
                <div className="flex items-center space-x-2 md:space-x-3 pl-3 md:pl-5 border-l border-[#453abc]/20">
                    <div className="hidden sm:block text-right">
                        <p className="text-xs lg:text-sm font-extrabold text-white leading-none mb-0.5 lg:mb-1 truncate max-w-[120px]">{user?.name || 'QA Engineer'}</p>
                        <p className="text-[8px] lg:text-[9px] font-semibold text-white/70 uppercase tracking-[0.2em] leading-none">{user?.role || 'SQA'}</p>
                    </div>
                    <img
                        src={`https://ui-avatars.com/api/?name=${user?.name || 'QA'}&background=453abc&color=ffffff`}
                        alt={user?.name}
