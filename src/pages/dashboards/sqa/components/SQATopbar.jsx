            ...
            <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-3 pl-6 border-l border-[#fa2742]/20">
                    <div className="text-right flex flex-col justify-center">
                        <p className="text-sm font-extrabold text-white leading-none mb-1">{user?.name || 'QA Engineer'}</p>
                        <p className="text-[9px] font-semibold text-white/70 uppercase tracking-[0.2em] leading-none">{user?.role || 'SQA'}</p>
                    </div>
                    <img
                        src={`https://ui-avatars.com/api/?name=${user?.name || 'QA'}&background=fa2742&color=373833`}
                        alt={user?.name}
