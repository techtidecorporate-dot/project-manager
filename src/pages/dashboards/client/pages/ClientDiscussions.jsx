...
const ClientDiscussions = () => {
    ...
    const threads = requests.filter(r => {
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-4xl font-black text-[#191a23] tracking-tight">Discussions</h1>
                    <p className="text-gray-500 font-bold text-sm">Project conversations with the team.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {threads.map((thread) => {
                        ...
                        return (
                            <div key={thread._id} className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                                <div className="p-6">
                                    ...
                                    <div className="space-y-3 max-h-64 overflow-y-auto mb-4 pr-2">
                                        {thread.messages && thread.messages.length > 0 ? (
                                                ...
                                                return (
                                                    <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                        <div className={`max-w-[80%] p-3 rounded-2xl ${isMe
                                                            ...
                                                            <p className={`text-[10px] font-black uppercase mt-1 ${isMe ? 'text-white/40' : 'text-gray-400'}`}>
                                            ...
                                            <div className="text-center py-4 text-gray-400 font-medium italic">No messages yet.</div>
                                        )}
                                    </div>
                ...
                <div className="py-24 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
                    ...
                    <h3 className="text-xl font-bold text-[#191a23]/40">No Discussions Yet</h3>
                    <p className="text-sm text-gray-400 font-bold mt-2">Start a discussion via the Support Tickets page.</p>
                </div>
            )}
        </div>
    );
