    ...
    return (
        <div className="space-y-8 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-4xl font-black text-[#191a23] tracking-tight">Support Desk</h1>
                    <p className="text-gray-500 font-bold text-sm">All your requests and complaints â€” in one place.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-gradient-to-br from-[#453abc] to-[#60c3e3]  text-white rounded-2xl shadow-xl hover:shadow-2xl font-black transition-all text-sm flex items-center gap-2 active:scale-95"
                    >
                        <Plus size={18} />
            ...
            <div className="grid gap-6">
                {filteredTickets.map((ticket) => {
                    ...
                    return (
                        <div
                            key={ticket._id}
                            className={`bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden transition-all ${isExpanded ? 'ring-2 ring-[#453abc]/10 shadow-xl' : 'hover:shadow-md'}`}
                        >
                            <div
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <h3 className="font-black text-[#191a23] text-xl leading-tight group-hover:text-[#453abc] transition-colors">{ticket.title}</h3>
                            ...
                            {isExpanded && (
                                <div className="p-5 border-t border-gray-50 bg-gray-50/30 space-y-4 max-h-80 overflow-y-auto" onClick={e => e.stopPropagation()}>
                                            ...
                                            return (
                                                <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`max-w-[80%] p-6 rounded-[24px] ${isMe
                                                        ...
                                                        : 'bg-white border border-gray-100 text-[#191a23] rounded-tl-none shadow-sm'
                                                        }`}>
                                                        <p className="text-sm font-bold leading-relaxed">{msg.content}</p>
                                                        <div className={`flex items-center mt-3 gap-2 text-[10px] font-black uppercase ${isMe ? 'text-white/40' : 'text-gray-300'}`}>
                                                            <Clock size={10} />
                                                            <span>{isMe ? 'You' : (typeof msg.sender === 'object' ? msg.sender?.name : 'Admin')} â€¢ {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                    ...
                                    {ticket.status !== 'Closed' && ticket.status !== 'Resolved' && (
                                        <div className="flex items-center gap-2 pt-2">
                                            <input
                                                ...
                                                className="flex-1 bg-white border-2 border-transparent focus:border-[#453abc] rounded-xl px-4 py-2.5 text-sm font-bold outline-none transition-all shadow-sm"
                                            />
                                            <button
                                                onClick={() => handleReply(ticket._id)}
                                                disabled={isThisSubmitting || !currentReply.trim()}
                                                className="bg-[#453abc] text-white p-2.5 rounded-xl hover:shadow-lg active:scale-95 transition-all disabled:opacity-40 shrink-0"
                                            >
                                                {isThisSubmitting
                                    ...
                                    {(ticket.status === 'Closed' || ticket.status === 'Resolved') && (
                                        <p className="text-center text-xs font-black text-gray-300 uppercase tracking-[0.15em] pt-2">Case {ticket.status} â€” contact support to reopen</p>
                                    )}
                                </div>
                            )}
                        </div>
                ...
                {filteredTickets.length === 0 && (
                    <div className="py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
                        ...
                        <h3 className="text-xl font-black text-[#191a23]">Silent Inbox</h3>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#191a23]/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl p-4 md:p-5 w-full max-w-[95vw] md:max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-black text-[#191a23] tracking-tight">Initiate Ticket</h3>
                        ...
                        <form onSubmit={handleCreateTicket} className="space-y-3">
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#453abc] rounded-xl px-4 py-2.5 text-sm font-bold outline-none transition-all"
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                <div className="space-y-1.5">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#453abc] rounded-xl px-2 py-2.5 text-xs font-bold outline-none transition-all appearance-none"
                                    >
                            ...
                            <div className="space-y-1.5">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#453abc] rounded-xl px-4 py-2.5 text-sm font-bold outline-none transition-all resize-none"
                                />
                            </div>

                            <button type="submit" className="w-full py-2.5 bg-[#453abc] text-white font-black rounded-xl hover:shadow-lg transition-all active:scale-95 text-xs uppercase tracking-widest">
                                Submit Inquiry
                            </button>
                        </form>
                    </div>
