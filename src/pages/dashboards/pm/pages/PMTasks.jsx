...
import {
    CheckSquare, Plus, X, Edit2, Trash2, Search, Eye,
    FolderKanban, User, Calendar, AlertCircle, CheckCircle2
} from 'lucide-react';
...
const PMTasks = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        project: '', assignedTo: '', dueDate: '', tags: ''
    });
    ...
    const handleOpenModal = (task = null) => {
        if (task) {
            ...
            setEditingTask(null);
            setFormData({ title: '', description: '', status: 'Todo', priority: 'Medium', project: '', assignedTo: '', dueDate: '', tags: '' });
        }
        setShowModal(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/tasks${editingTask ? '/' + editingTask._id : ''}`, {
    ...
    return (
        <div className="space-y-8 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-[#191a23] tracking-tight">Task Manager</h2>
                    <p className="text-[#191a23]/60 font-medium text-sm">Create, assign, and track tasks across all projects.</p>
                </div>
                                ...
                                <tr key={t._id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <span className="text-xs font-bold text-[#191a23]/60 flex items-center gap-1.5">
                                            <FolderKanban size={12} />{t.project?.title || '---'}
                                        </span>
                                    </td>
                                    <td className="py-4 px-6">
                                        {t.assignedTo ? (
                                            <span className="text-xs font-bold text-[#191a23]/60 flex items-center gap-1.5">
                                                <User size={12} />{t.assignedTo.name || 'Unknown'}
                                            </span>
                                        ) : <span className="text-xs text-[#191a23]/30 italic">Unassigned</span>}
                                    </td>
                                    <td className="py-4 px-6">
                                        <span className={`px-3 py-1 rounded-lg text-[10px] font-black ${statusColors[t.status] || 'bg-gray-100 text-gray-600'}`}>{t.status}</span>
                                    </td>
                                    <td className="py-4 px-6">
                                        <span className={`px-3 py-1 rounded-lg text-[10px] font-black ${priorityColors[t.priority] || 'bg-gray-100 text-gray-600'}`}>{t.priority}</span>
                                    </td>
                                    <td className="py-4 px-6">
                                        {t.dueDate ? (
                                            <span className="text-xs font-bold text-[#191a23]/60 flex items-center gap-1.5">
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="flex items-center justify-center gap-2">
                                            <button onClick={() => setDetailTask(t)}
                                                className="p-2 text-[#191a23]/40 hover:text-[#453abc] hover:bg-[#453abc]/10 rounded-lg transition-all" title="View Details"><Eye size={16} /></button>
                                            <button onClick={() => handleOpenModal(t)}
                                                className="p-2 text-[#191a23]/40 hover:text-blue-500 hover:bg-blue-500/10 rounded-lg transition-all"><Edit2 size={16} /></button>
                                            <button onClick={() => handleDelete(t._id)}
                                                className="p-2 text-[#191a23]/40 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"><Trash2 size={16} /></button>
                                        </div>
                                    </td>
                                </tr>
                            )) : (
                                <tr><td colSpan="7" className="py-16 text-center text-[#191a23]/30 italic font-bold uppercase tracking-widest text-xs">No tasks found</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="fixed inset-0 bg-[#191a23]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[32px] p-4 md:p-6 lg:p-8 shadow-2xl max-w-[95vw] md:max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-2xl font-black text-[#191a23] tracking-tight">{editingTask ? 'Edit Task' : 'Create New Task'}</h3>
                            <button onClick={() => setShowModal(false)} className="w-10 h-10 bg-white rounded-full flex items-center justify-center hover:bg-gray-100 transition-colors"><X size={20} /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea name="description" value={formData.description} onChange={handleInputChange} rows="3"
                                    className="w-full bg-[#f5f5f5] border-2 border-transparent focus:bg-white focus:border-[#453abc] rounded-xl py-3 px-4 text-[#191a23] outline-none transition-all font-bold resize-none" required />
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                                <div className="space-y-2">
                                    ...
                                    <select name="status" value={formData.status} onChange={handleInputChange}
                                        ...
                                        {['Todo', 'In Progress', 'In Review', 'Completed'].map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-black text-[#191a23] uppercase tracking-widest ml-1">Tags</label>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4">
                                <button type="submit"
                                    className="w-full sm:flex-1 bg-gradient-to-r from-[#453abc] to-[#60c3e3] text-white py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:shadow-lg transition-all">
