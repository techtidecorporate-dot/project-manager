    ...
    return (
        <div className="space-y-8 pb-20">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold text-[#373833] tracking-tight">Project Division</h2>
                    <p className="text-[#373833]/60 font-medium text-sm">Organize projects into actionable phases.</p>
                </div>
                <button
            ...
            <div className="grid gap-8">
                {projects.map((project) => (
                    <div key={project._id} className="bg-white rounded-3xl p-6 shadow-xl border border-[#373833]/5 relative overflow-hidden">
                        <div className="flex items-center justify-between mb-6 border-b border-[#373833]/5 pb-3">
                            <h3 className="text-xl font-black text-[#373833]">{project.title}</h3>
                            <button
                                onClick={() => openEditModal(project)}
                                className="text-xs font-bold bg-[#373833]/5 hover:bg-[#fa2742] hover:text-[#373833] px-3 py-2 rounded-lg transition-colors flex items-center space-x-2"
                            >
                                <Edit size={14} />
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                            {project.phases && project.phases.length > 0 ? project.phases.map((phase, i) => (
                                <div key={i} className="bg-[#f5f5f5] p-4 rounded-2xl relative group hover:shadow-md transition-all border border-transparent hover:border-[#fa2742]/20">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center space-x-2">
                                            <span className="w-5 h-5 bg-[#373833] text-white rounded-full flex items-center justify-center text-[10px] font-bold">{i + 1}</span>
                                    ...
                                    <h4 className="font-bold text-[#373833] text-sm mb-1 line-clamp-1">{phase.name}</h4>

                                    <div className="space-y-1.5 text-[11px] text-[#373833]/70">
                                        <div className="flex items-center space-x-2">
                                            <User size={12} className="text-[#373833]/40" />
                                            <span className="truncate">Dev: {typeof phase.developer === 'object' ? phase.developer.name : getUserName(phase.developer)}</span>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <CheckCircle2 size={12} className="text-[#373833]/40" />
                                            <span className="truncate">SQA: {typeof phase.sqa === 'object' ? phase.sqa.name : getUserName(phase.sqa)}</span>
                                        </div>

                                        <div className="pt-2 mt-2 border-t border-[#373833]/5 text-[9px] space-y-1">
                                            <div className="flex justify-between">
                                                <span className="text-[#373833]/40 uppercase font-black">Status</span>
                                                <span className={clsx(
                                                    "font-bold",
                                                    phase.priority === 'High' ? 'text-[#fa2742]' :
                                                        phase.priority === 'Medium' ? 'text-blue-500' : 'text-gray-400'
                                                )}>{phase.priority || 'Medium'}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-[#373833]/40 uppercase font-black">Timeline</span>
                                ...
                                <div className="col-span-full py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                    <p className="text-[#373833]/40 font-bold">No phases defined for this project.</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {isAddDivisionOpen && (
                <div className="fixed inset-0 bg-[#373833]/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center p-6 border-b border-[#373833]/5">
                            <h3 className="text-xl font-black text-[#373833]">Divide Project into Phases</h3>
                            <button onClick={() => setIsAddDivisionOpen(false)}><X size={24} className="text-[#373833]/60 hover:text-[#373833]" /></button>
                        </div>

                        <div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar">
                            <div>
                                <label className="text-xs font-bold uppercase text-[#373833]/60">Select Project</label>
                                <div className="relative mt-1">
                                    <input
                                        ...
                                        className="w-full bg-[#f5f5f5] p-3 rounded-xl outline-none focus:ring-2 focus:ring-[#fa2742] font-bold text-[#373833] pr-10"
                                    />
                                    <ChevronDown
                                        className="absolute right-3 top-1/2 -translate-y-1/2 text-[#373833]/40 cursor-pointer hover:text-[#fa2742]"
                                        size={20}
                                        onClick={() => setIsProjectDropdownOpen(!isProjectDropdownOpen)}
                                    />

                                    {isProjectDropdownOpen && (
                                        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-[#373833]/10 max-h-60 overflow-y-auto z-50">
                                                ...
                                                projects
                                                    ...
                                                    .map(p => (
                                                        <div
                                                            ...
                                                            className="p-3 hover:bg-[#fa2742]/5 cursor-pointer transition-colors border-b border-[#373833]/5 last:border-0"
                                                        >
                                                            <div className="font-bold text-[#373833] text-sm">{p.title}</div>
                                                            <div className="text-xs text-[#373833]/50">{p.client}</div>
                                                        </div>
                                                    ))
                                            ) : (
                                                <div className="p-4 text-center text-[#373833]/40 text-xs font-bold">
                                                    No projects found.
                                                </div>
                                            )}
                                        </div>
                            ...
                            <div className="bg-[#f9fafb] p-4 rounded-xl border border-[#373833]/5 space-y-3">
                                <h4 className="text-sm font-black text-[#373833]">Add Phase Details</h4>
                                <input
                                    value={currentPhase.name}
                                    onChange={e => setCurrentPhase({ ...currentPhase, name: e.target.value })}
                                    className="w-full bg-white p-2 rounded-lg border border-[#373833]/10 text-sm outline-none focus:border-[#fa2742]"
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-white p-2 rounded-lg border border-[#373833]/10 text-sm outline-none focus:border-[#fa2742] resize-none"
                                    rows="2"
                                ...
                                <div className="flex gap-2">
                                    <select
                                        ...
                                        className="w-1/2 bg-white p-2 rounded-lg border border-[#373833]/10 text-sm outline-none focus:border-[#fa2742]"
                                    >
                                ...
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-white p-2 rounded-lg border border-[#373833]/10 text-sm outline-none focus:border-[#fa2742]"
                                    />
                                    <button
                                        onClick={handleAddPhaseToNew}
                                        className="bg-[#373833] text-white px-4 rounded-lg hover:bg-[#fa2742] transition-colors font-bold text-xs uppercase"
                                    >
                                        Add
                                    </button>
                                </div>
                            ...
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                {newPhases.map((phase) => (
                                    <div key={phase.id} className="flex justify-between items-center bg-white p-3 rounded-lg shadow-sm border border-[#373833]/5">
                                        <div className="flex flex-col flex-1">
                                            <span className="text-sm font-bold text-[#373833]">{phase.name}</span>
                                            {phase.description && (
                                                <span className="text-xs text-[#373833]/60 italic mt-1">{phase.description}</span>
                                            )}
                                            <span className="text-xs text-[#373833]/50">Dev: {getUserName(phase.dev)} | SQA: {getUserName(phase.sqa)}</span>
                                            <span className="text-[10px] text-[#fa2742] font-black uppercase mt-1">{phase.priority} Priority | {phase.duration}d (D:{phase.devDays} S:{phase.sqaDays})</span>
                                        </div>
                        ...
                        <div className="p-6 border-t border-[#373833]/5 bg-gray-50/50">
                            <button
                                onClick={handleSaveDivision}
                                className="w-full bg-[#fa2742] text-[#373833] font-black py-4 rounded-xl uppercase tracking-widest text-xs hover:opacity-90 shadow-lg active:scale-95 transition-all"
                            >
                                Confirm Division
                            </button>
                        </div>
            ...
            {isEditModalOpen && editingProject && (
                <div className="fixed inset-0 bg-[#373833]/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center p-6 border-b border-[#373833]/5">
                            <h3 className="text-xl font-black text-[#373833]">Edit Phases: {editingProject.title}</h3>
                            <button onClick={() => setIsEditModalOpen(false)}><X size={24} className="text-[#373833]/60 hover:text-[#373833]" /></button>
                        </div>

                        <div className="p-6 overflow-y-auto flex-1 space-y-4 custom-scrollbar">
                            <div className="bg-[#f9fafb] p-4 rounded-xl border border-[#373833]/5 space-y-3">
                                <h4 className="text-sm font-black text-[#373833]">Add New Phase</h4>
                                <input
                                ...
                                <button
                                    ...
                                    className="w-full bg-[#373833] text-white py-2 rounded-lg hover:bg-[#fa2742] transition-colors font-bold text-xs uppercase"
                                >
                                    Add Phase
                                </button>
                            </div>
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto">
                                {editingProject.phases && editingProject.phases.map((phase, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-lg shadow-sm border border-[#373833]/5">
                                        <div className="flex flex-col flex-1">
                                            <span className="text-sm font-bold text-[#373833]">{phase.name}</span>
                                            {phase.description && (
                                                <span className="text-xs text-[#373833]/60 italic mt-1">{phase.description}</span>
                                            )}
                                            <span className="text-xs text-[#373833]/50">Dev: {typeof phase.developer === 'object' ? phase.developer.name : getUserName(phase.developer)}</span>
                                            <span className="text-[10px] text-[#fa2742] font-black uppercase mt-1">{phase.priority || 'Medium'} Priority | {phase.duration || 0}d (D:{phase.devDays || 0} S:{phase.sqaDays || 0})</span>
                                        </div>
